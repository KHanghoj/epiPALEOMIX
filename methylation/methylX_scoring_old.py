...
def parse_args(argv):
    ...
    parser.add_argument('--out', help='...',
                        default='out_mapmethylX_scoring.txt')
...
def writetofile(score_overall, f_name):
    ''' dfs '''
...
def getmeanvalues(idx_standard, last_size,
                  dic_top, dic_lower, score_overall):
    temp_top = []
    temp_low = []
    for idx in idx_standard:
        for key in dic_lower[idx]:
            if key >= 0 and key < last_size:
                top = (dic_top[idx][key]+0.0001)
                low = (dic_lower[idx][key])
                # low = (dic_lower[idx][key]+0.0001)
                # temp_top.append(top)
                # temp_low.append(low)
                temp_low.append(top/low)
    if temp_low:
        # score_overall.append(sum(temp_top)/sum(temp_low))
        score_overall.append(sum(temp_low)/len(temp_low))
...
def main(argv):
    ...
    for chrom, start, end in read_bed(args):
        ...
        if check_lst_dic(lst_dic_lastpos):
            call_minus_ms(idx_standard, last_pos, lst_dic_lastpos,
                          start, dic_top, dic_lower)
        if check_lst_dic(dic_lower):
            getmeanvalues(idx_standard, last_size,
                          dic_top, dic_lower, score_overall)
        dic_top = create_lst_dic()
        dic_lower = create_lst_dic()
        lst_dic_lastpos = create_lst_dic(nested=True)
    ...
    if check_lst_dic(dic_lower):
        getmeanvalues(idx_standard, last_size,
                      dic_top, dic_lower, score_overall)
    writetofile(score_overall, args.out)
    samfile.close()
    fasta.closefile()
    return 0
