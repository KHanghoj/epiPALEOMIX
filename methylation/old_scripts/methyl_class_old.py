...
class Methyl_Level(object):
    ...
    def update(self, record):
        ...
        if self.record.tid != self.last_tid:
            ...
            if self.last_tid:
                ...
                self.ms_reverse()
                self.reset_dicts()
        # take all reverse lastpos greater than curr pos
        if self.check_lst_dic() and self._max_lst_dic() < self.record.pos:
            self.ms_reverse()
    ...
    def ms_reverse(self):
        for idx in self.indexoflist:
            for key in sorted(self.lst_dic_lastpos[idx].keys()):
