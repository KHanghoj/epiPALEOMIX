...
analpertis <- function(tissuefilespath, sample){
    ...
    forappend <- do.call(rbind, parallel::mclapply(CUTOFFVAL,
                                                   ...
                                                   sample=sample,
                                                   mc.cores=12))
    write.table(forappend, file='BIGDF_RRBSlinearmodel_MERGEDRRBSDATA.txt',append=T,row.names=F,col.names=F,quote=F,sep='\t')
}
...
runanalyses<- function(samplepath){
    ...
    parallel::mclapply(tissuefiles, analpertis, sample=sample,mc.cores=3)
}

require(data.table)
require(plyr)

CUTOFFVAL <- c(seq(0,250,10),300,350)
