...
concattis <- function(sampleidx){
    ...
    do.call(rbind, parallel::mclapply(TISSUE.N, analpertis, sample=sample, mc.cores=1))
}
...
##tissuefiles <- tissuefiles[grep('Osteobl|Bctestisn30', tissuefiles)]
...
write.table(bigdf, file='only_Alt_RRBSlinearmodel_newmethyl.txt',row.names=F,col.names=T,quote=F,sep='\t')
...
msecond = m[(nrow(m)/2):nrow(m),]
      pdf('oneplotnew.pdf')
	print(ggplot(mfirst,aes(compared, rsquared, col=cov_cpg_cutoff))+geom_point() +
      ...
      pdf('twoplotnew.pdf')
	print(ggplot(msecond,aes(compared, rsquared, col=cov_cpg_cutoff))+geom_point() +
      		           theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.5,size=5),
                	   legend.position='bottom')+coord_flip()+guides(col=guide_legend(nrow=3)));dev.off()
