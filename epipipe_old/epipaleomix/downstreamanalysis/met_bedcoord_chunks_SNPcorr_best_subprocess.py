...
import numpy as np
import subprocess,gzip, re, sys, argparse
FMT='{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format
...
def writetofile(f_out, ds, cs, tot, bedc):
    ...
    f_out.stdin.write(FMT(chrom, start, end, dssum, totsum,  rate, len(tot), vards, np.var(cs), sum(cs)))


def methylepipal(args):
    f_in = subprocess.Popen(('zcat', args.metpath),stdout=subprocess.PIPE)
    f_in.stdout.next() # removes the header
    ds, cs, tot = [], [], []
    checked = 0
    ...
    fileout = open(args.out, 'wb')
    f_out = subprocess.Popen(('gzip', '-c'), stdout=fileout, stdin=subprocess.PIPE)
    f_out.stdin.write(FMT(*h.split('\t')))
    for line in f_in.stdout:
        if checked % 500000 == 0:
            sys.stderr.write('checked {} methylated sites\n'.format(checked))
