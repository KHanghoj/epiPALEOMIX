...
p2
invisible(dev.off())
df = cbind('count'=0:maxtv,'newpts'=newpts, 'pred_ratio'=predvec, 'pred_ratio_inv'=1/predvec)
df[,4][is.infinite(df[,4])] = 0
# write.table(df, file='~/Desktop/pred.txt',row.names=F,col.names=F,quote=F,sep='\t')
# write.table(df, file='~/Desktop/pred_norm.txt',row.names=F,col.names=T,quote=F,sep='\t')

# o_file = sprintf('gccorrect_model_%s',pat)
# o_file = output_file
write(df[,4],stdout(), ncolumn=1)
# write.table(df[,c(1,4)], file=o_file,row.names=F,col.names=F,quote=F,sep='\t')