...
class GeneralExecuteNodenew(CommandNode):
    def __init__(self, anal, aux_python, dat_bam, bedinfo, dependencies=()):
        call = ['python', '%(AUX_PYTHON)s', '%(IN_BAM)s',
                '%(IN_BED)s', '%(OUT_FILEPATH)s']
        # bam_name, anal_name, outfmt, options = deepcopy(analysis_options)
        bed_name, bed_path = bedinfo
        destination = \
            os.path.join(dat_bam.o_path,
                         dat_bam.fmt(dat_bam.base_bam, bed_name,
                                     dat_bam.anal_name))
        if not dat_bam[anal].get('Apply_GC_Correction', True):
            options.pop('--GCmodel', None)

        for option, argument in dat_bam.retrievedat[anal].iteritems():
            if isinstance(option, str) and option.startswith('-'):
                call.extend((option, argument))
        cmd = AtomicCmd(call,
                        AUX_PYTHON=aux_path,
...
class General_Plot_Nodenew(CommandNode):
    def __init__(self, aux_path, infile, anal_name, dependencies=()):
        call = ['Rscript', '%(AUX_R)s', '%(IN_TXT)s', '%(OUT_FILEPATH)s']
        outfile = os.path.splitext(os.path.splitext(infile)[0])[0]+'.pdf'
        if anal_name in EXTRAS:
